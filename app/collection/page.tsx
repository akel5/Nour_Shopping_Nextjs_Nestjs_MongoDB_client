...
export default function CollectionPage() {
  ...
  // --- פונקציות לטיפול במודל (עריכה בלבד) ---
  ...
  // פונקציית שמירה (רק עריכה)
  const handleSaveProduct = async (formData: Omit<Product, '_id'> & { categoryName: string }, productId: string | null) => {
    if (!productId) return; // הגנה - לא אמור לקרות במצב עריכה
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {isModalOpen && productToEdit && ( // פתח רק אם יש מוצר לערוך
        <ProductModal
          productToEdit={productToEdit}
          categoryName={productToEdit.categoryName} // העבר את שם הקטגוריה מהמוצר הנבחר
      ...
      <main className="container mx-auto p-4 py-10">
        <h1 className="text-2xl font-bold text-gray-800 mb-8">כל המוצרים</h1>
        ...
        {!isLoading && !error && (
          products.length > 0 ? (
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
             ...
             <p className="text-center text-gray-600">עדיין אין מוצרים בחנות. {isAdmin && "ניתן להוסיף מוצרים דרך עמודי הקטגוריות."}</p>
          )
        )}
      </main>
    </div>
